...
if($dataload1 =="true")
{
  ...
  {
    ?>
    @extends('DataProcessing/DivisionDataProcessing')
  ...
  if(!empty($completestatus))
  {
    foreach($completestatus as $r)
    {
      ...
      if(empty($checkstatus))
      {
        ...
        else if($mont >='05' and $mont <='07')
        {
          if(($month >='05' and $month <='07'))
          {                
            //echo $month;
          }
          else
        ...
        else if($mont >='08' and $mont <='09')
        {
          if(($month >='08' and $month <='09'))
          {                
            //echo $month;
          }
          else
...
$div_id =0;
$area_id=0;
$division_name = $divisiondata[0]->division_name;;
$div_id = $divisiondata[0]->division_id;
?>
  <div class="row">
    <div class="panel panel-default">
      <div class="header">
        <h3><i>Division Current Dashboard:</i></h3>
        <br />
      </div>
      <div id="rcorners">
        <div class="row">
          ...
          <?php
             $cmonth = date('m');
              ...
              $score =0;
               $year='';
               $quarter='';
                 $checkdata= DB::select(DB::raw("select year,quarterly from mnwv2.monitorevents where division_id='$a_id' and areacompletestatus =0 group by year,quarterly order by year DESC, quarterly DESC LIMIT 1"));
                 ...
                 if(!empty($checkdata))
                 {
                   foreach($checkdata as $row)
                   {
                     ...
                     $cdata= DB::select(DB::raw("select * from mnwv2.monitorevents where division_id='$a_id' and year='$year' and quarterly='$quarter'"));
                     ...
                     if($score !=0)
                     {
                      ...
                      <tr>
                        ...
                        <a style="color: black;" class="btn btn-danger" href="DivisionCurrentDashboard?event=<?php echo $year."-".$quarter; ?>"><?php echo $year."-".$quarter; ?></a></td>
                        <td style="text-align: center;">
      ...
      <table style="text-align: center;font-size:13" class="table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
       ...
       //echo $year."/".$quarter."*".$a_id;
        $division = DB::select(DB::raw("select year,quarterly,month from mnwv2.monitorevents where division_id='$a_id' and year='$year' and quarterly='$quarter' group by year,quarterly,month order by month ASC"));
      if(!empty($division))
      {
        foreach($division as $row)
        {
          ...
          for($i=1; $i<=5;$i++)
          {
            ...
            $totalsectp =0;
            $totalscore =0;
            $monthwise = DB::select(DB::raw("select * from mnwv2.monitorevents where division_id='$a_id' and year='$year' and quarterly='$quarter' and month='$month'"));
            if(empty($monthwise))
            {
            ...
            if($i==1)
            {
              ...
              ?>
                <tr  style="height:20px; color:black;">
                  ...
                  <td style="color: black; text-align: center;"><a  style="color: black;" class="btn btn-danger" href="RegionWiseAcheivement?section=<?php echo $i.",".$year.",".$quarter.",".$month.",".$a_id; ?>"><?php echo "Section: ".$i; ?></a></td>
