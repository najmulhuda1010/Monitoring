...
<?php
  $mont = date('m');
  $yar = date('Y');
  $completestatus = DB::select(DB::raw("select * from mnw.monitorevents where area_id='$a_id'"));
  if(!empty($completestatus))
  {
    foreach($completestatus as $r)
    {
      ...
      $brc = $r->branchcode;
      $year = $r->year;
      $checkstatus = DB::select(DB::raw("select * from mnw.monitorevents where area_id='$a_id' and id='$eid' and areacompletestatus=1"));
      if(empty($checkstatus))
      {
        if($mont >='01' and $mont <='03')
        {
          ...
          {
            $updateareacompletestatus = DB::table('mnw.monitorevents')->where('id',$eid)->where('branchcode',$brc)->update(['areacompletestatus'=>1]);
          }
        }
    ...
    <div class="panel panel-default">
                   ...
                   foreach($checkdata as $row)
                   {
                     ...
                     if($score !=0)
                     {
                      ...
                      <tr>
                        ...
                        <a style="color: black;" class="btn btn-danger" href="CurrentDashboard?event=<?php echo $year."-".$quarter; ?>"><?php echo $year."-".$quarter; ?></a></td>
                        <td style="text-align: center;">
      ...
      <table style="text-align: center;font-size:13" class="table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
       ...
       //echo $year."/".$quarter."*".$a_id;
        $areas = DB::select(DB::raw("select year,quarterly,month from mnwv2.monitorevents where area_id='$a_id' and year='$year' and quarterly='$quarter' group by year,quarterly,month"));
      if(!empty($areas))
      {
