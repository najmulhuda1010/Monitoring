            ...
            <div class="card-body">
              <div class="row">
                <div class="col-md-12">
                  ...
                  <?php 
        $ct =0;
        $div = DB::select(DB::raw("select division_id from mnwv2.monitorevents where year='$year' and quarterly='$quarter' and month='$month' group by division_id order by division_id ASC"));
        if(!empty($div))
        {
          foreach ($div as $row) 
          {
            ...
            $sp =0;
            $qsp=0;
            $dataget = DB::select(DB::raw("select * from mnwv2.monitorevents where division_id='$division' and year='$year' and quarterly='$quarter' and month='$month'"));
          ...
          <table style="text-align: center;font-size:13" id="<?php echo "dv".$ct; ?>" class="table" cellspacing="0" width="100%">
            ...
            <tbody>
              <?php
              $area = DB::select(DB::raw("select region_id from mnwv2.monitorevents where division_id='$division' and year='$year' and quarterly='$quarter' group by region_id order by region_id ASC"));
              foreach ($area as $row) 
              {
                ...
                $totalspoint=0;
                $totalsectp =0;
                $eventid = DB::select(DB::raw("select * from mnwv2.monitorevents where division_id='$division' and year='$year' and quarterly='$quarter' and region_id='$regionid' and month='$month'"));
                if(!empty($eventid))
                {
              ...
              ?>
              <tr>
                <td width="50%" style="text-align: center"><a class="btn btn-light" target="_blank" href="MonthAreaWise?region=<?php echo $section.",".$regionid.",".$year.",".$quarter.",".$month; ?>"><?php echo $regionname; ?></a></td>
