...
$getdivision= DB::select( DB::raw("select division_id from branch where program_id=1 group by division_id order by division_id"));
if(!empty($getdivision))
{
	foreach($getdivision as $row)
	{
		$divid = $row->division_id;
		$getdivision= DB::select( DB::raw("select region_id from branch where division_id='$divid' and program_id=1 group by region_id order by region_id ASC"));
		if(!empty($getdivision))
		{
			foreach($getdivision as $row)
			{
